...
func binaryEncode(parameterStatus *parameterStatus, x interface{}) (bv []byte) {
	switch v := x.(type) {
	case []byte:
		bv = v
		return
	default:
		bv = encode(parameterStatus, x, oid.T_unknown)
		return
	}
}

func aliasEncode(parameterStatus *parameterStatus, x interface{}, kbtypOid oid.Oid) (value []byte) {
	...
	switch k {
		...
		case reflect.String:
			if oid.T_bytea == kbtypOid { return encodeBytea(parameterStatus.serverVersion, []byte(rv.String()))
...
func encode(parameterStatus *parameterStatus, x interface{}, kbtypOid oid.Oid) (value []byte) {
	rv := reflect.ValueOf(x)
	switch v := x.(type) {
	...
	case []byte:
		if oid.T_bytea == kbtypOid { return encodeBytea(parameterStatus.serverVersion, v)
		}else { return v }
	case string:
		if oid.T_bytea == kbtypOid { return encodeBytea(parameterStatus.serverVersion, []byte(v))
		}else{ return []byte(v) }
	case VarChar:
		if oid.T_bytea == kbtypOid { return encodeBytea(parameterStatus.serverVersion, []byte(v))
		}else{ return []byte(v) }
	...
	case sql.NullString:
		if oid.T_bytea == kbtypOid { return encodeBytea(parameterStatus.serverVersion, []byte(v.String))
	...
	case sql.Out://out参数
		...
		if isBoth {
			return encode(parameterStatus, rvOut.Elem().Interface(), kbtypOid)
		}
		return nil
	default: 
		return aliasEncode(parameterStatus, x, kbtypOid)
	}
	panic("not reached")
}
...
func textDecode(parameterStatus *parameterStatus, s []byte, typ oid.Oid, cn conn) (value interface{}) {
	...
	case oid.T_char, oid.T_varchar, oid.T_text:
		value = string(s)
		return
	case oid.T_varcharbyte, oid.T_nvarchar, oid.T_bpcharbyte, oid.T_nchar:
		value = string(s)
		return
	case oid.T_bytea:
		b, err := parseBytea(s)
	...
	case oid.T_timestamptz:
		...
		return
	case oid.T_timestamp, oid.T_datetime:
	...
	case oid.T_bit:
		value = s
		return
	case oid.T_int8, oid.T_int4, oid.T_int2, oid.T_smallint, oid.T_int, oid.T_bigint:
		i, err := strconv.ParseInt(string(s), 10, 64)
		if nil != err { errorf("%s", err) }
		value = i
		return i
	case oid.T_tinyint:
		sVal, _ := strconv.ParseUint(string(s), 10, 64)
		if cn.databaseMode == "sqlserver" {
			value = uint8(sVal)
		} else {
			value = int8(sVal)
		...
		return
	case oid.T_float4, oid.T_float8, oid.T_float, oid.T_real:
		f, err := strconv.ParseFloat(string(s), 64)
		if nil != err { errorf("%s", err) }
		value = f
		return
	case oid.T_numeric://numeric/decimal
		// num, _ := decimal.NewFromString(string(s))
		// return num
		return s
	case oid.T_money://money
...
func mustParse(f string, typ oid.Oid, s []byte) (pt time.Time) {
	str := string(s)
...
func (p *timestampParser) expect(str string, char byte, pos int) (){
	...
	if c := str[pos]
	char != c && nil == p.err { p.err = fmt.Errorf("expected '%v' at position %v; got '%v'", char, pos, c) }
}
...
func ParseTimestamp(currentLocation *time.Location, str string) (pt time.Time, err error) {
	...
	if tzStart := remainderIdx
	len(str) > tzStart && ('-' == str[tzStart] || '+' == str[tzStart]) {
		// 时区分隔符为'-' 或'+' (UTC的时区分隔符为+00)
		var tzSign int
		switch c := str[tzStart]
		c {
		case '-': tzSign = -1
		case '+': tzSign = +1
