...
func (sm3 *SM3) pad() (msg []byte) {
	msg = sm3.unhandleMsg; msg = append(msg, 0x80)
	blockSize := 64         // 写入消息直到消息长度(以位为单位)等于448
	for 56 != len(msg)%blockSize { msg = append(msg, 0x00); }
	// append message length
	msg = append(msg, uint8(sm3.length>>56&0xff)); msg = append(msg, uint8(sm3.length>>48&0xff))
	msg = append(msg, uint8(sm3.length>>40&0xff)); msg = append(msg, uint8(sm3.length>>32&0xff))
	msg = append(msg, uint8(sm3.length>>24&0xff)); msg = append(msg, uint8(sm3.length>>16&0xff))
	msg = append(msg, uint8(sm3.length>>8&0xff)); msg = append(msg, uint8(sm3.length>>0&0xff))

	if 0 != len(msg)%64 { panic("------SM3 Pad: error msgLen ="); }
	return
}
...
func (sm3 *SM3) update(msg []byte) () {
	...
	for 64 <= len(msg) {
		...
		i++ {
			...
			D = C; C = sm3.leftRotate(B, 9)
			B = A; A = TT1
			H = G; G = sm3.leftRotate(F, 19)
			F = E; E = sm3.p0(TT2)
		}
		for
		i := 16
...
func (sm3 *SM3) Reset() (){
	// Reset digest
	sm3.digest[0] = 0x7380166f; sm3.digest[1] = 0x4914b2b9
	sm3.digest[2] = 0x172442d7; sm3.digest[3] = 0xda8a0600
	sm3.digest[4] = 0xa96f30bc; sm3.digest[5] = 0x163138aa
	sm3.digest[6] = 0xe38dee4d; sm3.digest[7] = 0xb0fb0e4e
...
func (sm3 *SM3) Write(p []byte) (toWrite int, err error) {
	toWrite = len(p); sm3.length = sm3.length + uint64(len(p) * 8)
	msg := append(sm3.unhandleMsg, p...); nblocks := len(msg) / sm3.BlockSize()
	sm3.update(msg)
	// Update unhandleMsg
