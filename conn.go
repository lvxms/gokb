...
func (c *Connector) open(ctx context.Context) (cn *conn, err error) {
	...
	// 重置deadline
	if timeout, ok := o["connect_timeout"]
	"0" != timeout && ok { err = cn.c.SetDeadline(time.Time{}) }
	panicking = false
	...
	for {
		...
		case 'C', 'I', 'Z':
			if 'C' == t { rs.result, rs.tag = cn.parseComplete(rs.rb.string()) }
			break
		case 'T':
...
func setDatabaseModeOid(cn *conn) {
	...
	} else if cn.databaseMode == "oracle" {
...
func (cn *conn) simpleExec(q string) (res driver.Result, commandTag string, err error) {
	...
	for {
		t, r := cn.recv1()
		switch t {
		case 'C': res, commandTag = cn.parseComplete(r.string())
		case 'Z':
...
func (cn *conn) sendPmessage (q, stmtName string, v []driver.Value) (hasRet bool) {
	...
	for i, t := range v {
		...
		if sBind[i].isOut {
			...
			case *int:
				//sBind[i].typ = oid.T_int
				if GetPlatformBit() == 64 {
					//sBind[i].typ = oid.T_int8
				}else {
					//sBind[i].typ = oid.T_int4
				}
			case *int64, *sql.NullInt64:
				//sBind[i].typ = oid.T_bigint
			case *int32:
				//sBind[i].typ = oid.T_int
			case *int16:
				//sBind[i].typ = oid.T_smallint
			case *int8:
				//sBind[i].typ = oid.T_tinyint
			case *uint:
				if GetPlatformBit() == 64 {
					//sBind[i].typ = oid.T_uint8
				}else {
					//sBind[i].typ = oid.T_uint4
				}
			case *uint64:
				//sBind[i].typ = oid.T_uint8
			case *uint32:
			...
			case *[]byte:
				if cn.databaseMode == "sqlserver" {
					sBind[i].typ = oid.T_binary
				} else {
					if len(*((sBind[i].out.Dest).(*[]byte))) <= 32767 {
						//sBind[i].typ = oid.T_bytea
					}else {
						//sBind[i].typ = oid.T_blob
					}
				}
			case *float64, *sql.NullFloat64:
				//sBind[i].typ = oid.T_float8
			case *float32:
				//sBind[i].typ = oid.T_float4
			case *decimal.Decimal:
				//numeric/decimal
				//sBind[i].typ = oid.T_numeric
			case *bool:
				sBind[i].typ = oid.T_bool
			case *sql.NullBool:
				sBind[i].typ = oid.T_bit
			case *string:
				//sBind[i].typ = oid.T_bpcharbyte
				if len(*((sBind[i].out.Dest).(*string))) > 32767 {
					//sBind[i].typ = oid.T_clob
				}
			case *VarChar, *VarCharMax:
				sBind[i].typ = oid.T_varcharbyte
			case *NVarCharMax, *sql.NullString:
				sBind[i].typ = oid.T_nvarchar
			case *NChar:
				sBind[i].typ = oid.T_nchar
			case *time.Time:
				//time timestamp date
			case *DateTime1:
				sBind[i].typ = oid.T_datetime
			case *civil.Date:
				sBind[i].typ = oid.T_date
			case *civil.Time:
				sBind[i].typ = oid.T_time
			case *CursorString:
				sBind[i].typ = oid.T_refcursor
			}
...
func (cn *conn) recv() (t byte, rb *readBuf) {
	for {
		...
		case 'N':
			if n := cn.noticeHandler
			nil != n { n(parseError(rb)) }
		default: return
		}
	}
}
...
func isDriverSetting(key string) (state bool) {
	...
	case "binary_parameters": state = true
	default: state = false
	}
	return
}
...
func (st *stmt) Exec(v []driver.Value) (res driver.Result, err error) {
	...
	res, _, err = st.cn.readExecuteResponse("simple query", v, st.colTyps)
	return
}
...
func (st *stmt) exec(v []driver.Value) (err error) {
	...
	if cn.binaryParameters { cn.sendBinaryParameters(w, v);
		...
		for i, x := range v {
			if nil == x { w.int32(-1);
			}  else { b := encode(&cn.parameterStatus, x, st.paramTyps[i]);
				if b != nil {
					w.int32(len(b))
					w.bytes(b)
				} else {
...
func (cn *conn) parseComplete(commandTag string) (result driver.Result, s string) {
	// INSERT在后续处理
...
func (cn conn) ParseOutValues(rb *readBuf, bindParams []driver.Value, colTyps []fieldDesc) (err error) {
	...
	for i, _ := range sBind {
		if sBind[i].isOut {
			...
			case oid.T_int2, oid.T_smallint:
			...
			case oid.T_float4, oid.T_real:
				f, _ := strconv.ParseFloat(string(byteValues[outNum]), 64)
				*dest.(*float32) = float32(f)
			case oid.T_bytea, oid.T_blob:
				*dest.(*[]byte) = byteValues[outNum]
			case oid.T_binary, oid.T_varbinary:
				*dest.(*[]byte) = byteValues[outNum]
			case oid.T_bool:
				var isBool bool
			...
			case oid.T_varchar, oid.T_char, oid.T_bpchar, oid.T_text, oid.T_clob:
				switch dest.(type) {
				case *string:
			...
			case oid.T_timestamp, oid.T_timestamptz, oid.T_datetime:
				switch dest.(type) {
				case *time.Time:
			...
			case oid.T_refcursor:
				(*dest.(*CursorString)).CursorName = string(byteValues[outNum])
			case oid.T_numeric, oid.T_money://decimal
...
func (rs *rows) Next(dest []driver.Value) (err error) {
	...
	for {
		...
		case 'E':
			...
			panic(err)
		case 'C', 'I':
			if 'C' == t { rs.result, rs.tag = conn.parseComplete(rs.rb.string()) }
			...
			if rs.outParamInMultiRes {
				...
				for i := range dest {
					...
					if 0 == l {
						...
						switch typs {
						case oid.T_varchar, oid.T_char, oid.T_bpchar, oid.T_text, oid.T_varcharbyte, oid.T_nvarchar, oid.T_bpcharbyte, oid.T_nchar:
							dest[i] = ""
						default:
							dest[i] = nil
						}
...
func (cn *conn) readExecuteResponse(protocolState string, v []driver.Value, colTyps []fieldDesc) (res driver.Result, commandTag string, err error) {
	for {
		t, r := cn.recv1()
		switch t {
		case 'T', 'I':
